...
Total number of loci = 154 Total number of gene copies = 308
...
9 mis-assignments, but only 6 with a scaled-likelihood > 0.9 and two are Lehua, which only has two individuals in the baseline, so probably not great representation for that colony. Maybe also suggesting that the Lehua colony was formed by birds from Midway...?
...
93% of samples correctly assigned at 90% scaled likelihood.
...
Which would be 96.5% accurate assignment.
...
848 bycatch samples, most of which are assigned at high probability.
...
```
There are some serious outliers and then some less significant outliers. Take a closer look:
...
Interesting. So the Lehua assignments have the crazy z-scores. But we also had very few baseline samples for Lehua. Not a lot of confidence in those assignments.

The Laysan assignments that are between 3-4 z-scores could just be enough different because we only had 8 Laysan baseline samples? 

I'm also curious to what extent timing (and date of baseline samples) plays a role here.
...
tmp.samp$indiv <- gsub("BFAL", "", tmp.samp$indiv)
tmp.samp
           

bycatch_w_meta <- metadata_ss %>% 
  unite(col = "indiv", gtseq_run, id, sep = "_") %>%
  left_join(., assigned_z_removed) %>%
  select(-missing_loci)
...
# reference data
tmp.samp %>%
  left_join(., metadata_ss, by = c("indiv" = "MWVCRC#")) %>%
  write_csv("../data/processed/reference_banded_assignments_w_metadata.csv")
```
130 samples without probability of assignment - some of those are likely (possibly banded birds that were used as reference).
